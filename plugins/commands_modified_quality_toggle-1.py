...
async def start(client, message):
    ...
    await asyncio.sleep(DELETE_TIME)
    await k.delete()
    await m.edit_text("<b>ʏᴏᴜʀ ᴠɪᴅᴇᴏ / ꜰɪʟᴇ ɪꜱ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ᴅᴇʟᴇᴛᴇᴅ !!</b>")
    return


@Client.on_message(filters.command("resetlimit"))
async def reset_user_limit(bot, message):
    try:
        user_id = message.from_user.id
        await db.reset_file_count(user_id)
        await message.reply_text("✅ Your daily file limit has been reset!")
    except Exception as e:
        await message.reply_text("❌ Failed to reset your file limit.")
        logger.exception(e)
